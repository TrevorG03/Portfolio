...
let submitted = false;
//This variable will stop the next function from running if it's false
//I had the last two variables below the array, they seem to work fine here.
...
options: ['Natasha Romanoff', 'Five', 'Six', 'Seven']
},

];
//This is an array of objects that hold the data for each question, with three properties each, "name", "answer", and "options", the final of which is an array.
...
function renderQuestion() {
    ...
    submitted = false;
    //Making sure the submit is cleared for each new question
...
//One critical thing is that we don't need to create elements, then assign them variables, then append them, to populate a <div>
//We can just use .innerHTML on a variable referencing a <div>, then create the HTML elements we need just like we would in HTML.
...
//We use the options[] items inside the question objects for the innerHTML of the labels
//Below that we have a <p> with the id of "message"
}
...
function submitAnswer() {
    const msg = document.getElementById("message");
    //Sets a variable to reference the <p> in the form
    ...
    //Disables all the radio buttons when one is selected
    
    submitted = true;
    //Allows the nextQuestion function to complete when invoked
}
